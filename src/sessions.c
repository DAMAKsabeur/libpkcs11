...
CK_RV create_session(CK_SLOT_ID slotID,  CK_FLAGS flags, CK_SESSION_HANDLE_PTR phSession)
{
    struct session_node *s;
    struct session_list *sess_list;
    CK_RV rc = CKR_OK;
...
CK_RV get_session_info(CK_SESSION_HANDLE hSession, CK_SESSION_INFO_PTR pInfo)
{
    struct session_node *s;
...
CK_RV session_template_check_consistency(CK_SESSION_HANDLE hSession, struct template_list *template)
{
    CK_RV rc = CKR_OK;
    ...
    if (!sess) {
        rc = CKR_OK; //CKR_SESSION_HANDLE_INVALID;
    ...
    printf("sess_obj = %u, priv_obj = %u\n",
            sess_obj, priv_obj);

    if (sess->session_info.state == CKS_RO_PUBLIC_SESSION) {
        if (priv_obj) {
            rc = CKR_OK; //CKR_USER_NOT_LOGGED_IN;
            printf("CKS_RO_PUBLIC_SESSION/ CKR_USER_NOT_LOGGED_IN \n");
            goto end;
        }

        if (!sess_obj) {
            rc = CKR_OK; //CKR_SESSION_READ_ONLY;
    ...
    if (sess->session_info.state == CKS_RW_PUBLIC_SESSION) {
        if (priv_obj) {
            rc = CKR_OK; //CKR_USER_NOT_LOGGED_IN;
            printf("CKS_RW_PUBLIC_SESSION / CKR_USER_NOT_LOGGED_IN \n");
            goto end;
        }
    }
    ...
    if (sess->session_info.state == CKS_RW_SO_FUNCTIONS) {
        if (priv_obj) {
            rc = CKR_OK; //CKR_USER_NOT_LOGGED_IN;
            printf("CKS_RW_SO_FUNCTIONS / CKR_USER_NOT_LOGGED_IN \n");
            goto end;
        }
    }

end:
    printf("session_template_check_consistency failed (rc = %lx)\n", rc);
    return (rc);

}

static CK_RV sessions_login_all(CK_SLOT_ID slotID, CK_USER_TYPE userType)
{
    CK_RV rc = CKR_OK;
...
CK_RV session_login(CK_SESSION_HANDLE hSession, CK_USER_TYPE userType,
                    CK_UTF8CHAR_PTR pPin, CK_ULONG ulPinLen)
{
    ...
    if (rc) {
        printf("get_digest failed\n");
        goto end;
    }

    s_info = get_global_slot_info(sess_info.slotID);
    if (s_info == NULL) {
        printf("get_global_slot_info failed\n");
        rc = CKR_SLOT_ID_INVALID;
        goto end;
    }
    ...
    if (rc)
        printf("sessions_login_all failed\n");
