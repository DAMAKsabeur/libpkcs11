...
CK_RV C_GetSlotList(CK_BBOOL tokenPresent, CK_SLOT_ID_PTR pSlotList, CK_ULONG_PTR pulCount)
{
    ...
    /* only support 1 slot which is NXP_SLOT */
    if (*pulCount >= SLOT_COUNT) {
        pSlotList[0] = NXP_S32G_HSM_SLOT_ID;
        slot_info = get_global_slot_info(NXP_S32G_HSM_SLOT_ID);
        slot_info->slot_id = NXP_S32G_HSM_SLOT_ID;
    } else
...
CK_RV C_GetSlotInfo(CK_SLOT_ID slotID, CK_SLOT_INFO_PTR pInfo)
{
    ...
    /* Currently only one slot is added i.e. NXP_SLOT
      * In order to add another slot, need to add 2 files
      * just like nxp_slot.c and nxp_slot.h in src folder
      * and add a case for that slot here */
    switch (slotID) {
        case NXP_S32G_HSM_SLOT_ID:
            Get_NXP_SlotInfo(pInfo);
            break;
        default:
...
CK_RV C_GetMechanismList(CK_SLOT_ID slotID, CK_MECHANISM_TYPE_PTR pMechanismList, CK_ULONG_PTR pulCount)
{
    ...
    switch (slotID) {
        case NXP_S32G_HSM_SLOT_ID:
            rc = Get_NXP_MechanismList(pMechanismList, pulCount);
            break;
        default:
            rc = CKR_SLOT_ID_INVALID;
    }
