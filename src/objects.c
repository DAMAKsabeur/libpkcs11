...
static CK_RV key_object_set_default_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    CK_RV rc = CKR_OK;
...
static CK_RV pubk_add_default_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    /* To satisfy compiler */
    op_type = op_type;
...
static CK_RV rsa_pubk_add_default_attr(struct template_list *tmpl_list,    CK_ULONG op_type)
{
    CK_RV rc;
    /* To satisfy compiler */
    op_type = op_type;
...
static CK_RV ecc_pubk_add_default_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    CK_RV rc;
    /* To satisfy compiler */
    op_type = op_type;
...
static CK_RV privk_add_default_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    /* To satisfy compiler */
    op_type = op_type;
...
static CK_RV rsa_privk_add_default_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    CK_RV rc;
...
static CK_RV ecc_privk_add_default_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    CK_RV rc;
...
static CK_RV attribute_check_required_base_attributes(struct template_list *template, CK_ULONG op_type)
{
    CK_ATTRIBUTE *attr;
    CK_BBOOL found;
...
CK_RV
attribute_validate_base_attributes(CK_ATTRIBUTE *attr, CK_ULONG op_type)
{
    CK_BBOOL value;
    if (!attr) {
...
static CK_RV attribute_key_object_check_required_attributes( struct template_list *template, CK_ULONG op_type)
{
    CK_ATTRIBUTE * attr = NULL;
    CK_BBOOL    found;
...
static CK_RV attribute_key_object_validate(CK_ATTRIBUTE *attr, CK_ULONG op_type)
{
    CK_BBOOL value;
    switch (attr->type) {
        case CKA_KEY_TYPE:
...
static CK_RV attribute_pubk_check_required_attributes(struct template_list *template, CK_ULONG op_type)
{
...
static CK_RV attribute_pubk_validate(CK_ATTRIBUTE *attr, CK_ULONG op_type)
{
    CK_BBOOL value;
    switch (attr->type) {
        case CKA_SUBJECT:
...
static CK_RV attribute_privk_check_required_attributes(struct template_list *template, CK_ULONG op_type)
{
    //CKO_PRIVATE_KEY has no required attributes
...
static CK_RV attribute_privk_validate(CK_ATTRIBUTE *attr, CK_ULONG op_type)
{
    CK_BBOOL value;
    switch (attr->type) {
        case CKA_SUBJECT:
...
static CK_RV attribute_rsa_pubk_check_required_attributes(struct template_list *template, CK_ULONG op_type)
{
    CK_ATTRIBUTE *attr = NULL;
...
static CK_RV attribute_rsa_pubk_validate(CK_ATTRIBUTE_PTR attr, CK_ULONG op_type)
{
    switch (attr->type) {
        case CKA_MODULUS_BITS:
            if (op_type == OP_GENERATE) {
...
static CK_RV attribute_rsa_privk_check_required_attributes(struct template_list *template, CK_ULONG op_type)
{
    CK_ATTRIBUTE *attr = NULL;
    CK_BBOOL   found;
...
static CK_RV attribute_rsa_privk_validate(CK_ATTRIBUTE_PTR attr, CK_ULONG op_type)
{
    switch (attr->type) {
        case CKA_MODULUS:
        case CKA_PRIVATE_EXPONENT:
...
static CK_RV attribute_ec_pubk_check_required_attributes(struct template_list *template, CK_ULONG op_type)
{
    CK_ATTRIBUTE *attr = NULL;
    CK_BBOOL   found;
...
static CK_RV attribute_ec_pubk_validate(CK_ATTRIBUTE_PTR attr, CK_ULONG op_type)
{
    switch (attr->type) {
        case CKA_EC_PARAMS:
...
static CK_RV attribute_ec_privk_check_required_attributes(struct template_list *template, CK_ULONG op_type)
{
    CK_ATTRIBUTE *attr = NULL;
    CK_BBOOL   found;
...
static CK_RV attribute_ec_privk_validate(CK_ATTRIBUTE_PTR attr, CK_ULONG op_type)
{
    switch (attr->type) {
        case CKA_EC_PARAMS:
            if (op_type == OP_CREATE)
...
static CK_RV template_merge(struct template_list *dest, struct template_list **src)
{
    CK_RV rc = CKR_OK;
 ...
 */
static CK_RV p11_template_add_default_common_attr(struct template_list *tmpl_list, CK_ULONG op_type)
{
    /* To satisfy compiler */
    op_type = op_type;
...
static CK_RV p11_template_add_default_attr(OBJECT *obj, CK_ULONG op_type)
{
    CK_RV rc;
...
static CK_RV p11_template_add_attr(struct template_list *template_list,
...
CK_BBOOL p11_template_attribute_find(struct template_list *template, CK_ATTRIBUTE_TYPE type,
		                             CK_ATTRIBUTE **attr)
{
    struct template_node *tmpl_node;
...
CK_BBOOL template_is_modifiable_set(struct template_list *template)
{
...
CK_BBOOL template_is_token_object(struct template_list *template)
{
...
CK_BBOOL template_is_session_object(struct template_list *template)
{
    CK_BBOOL rc;
...
CK_RV template_destroy_template_list(struct template_list *template)
{
    CK_RV rc = CKR_OK;
...
CK_RV template_create_template_list(CK_ATTRIBUTE_PTR pTemplate,
                                    CK_ULONG ulCount,
                                    struct template_list **tmpl_list)
{
    CK_RV rc = CKR_OK;
...
static CK_RV template_validate_attribute(CK_ATTRIBUTE *attr,
                                         CK_ULONG class, CK_ULONG subclass,
                                         CK_ULONG op_type)
{
...
CK_RV template_check_required_attributes(struct template_list *template, CK_ULONG class,
                                         CK_ULONG subclass, CK_ULONG op_type)
{
    if (class == CKO_PUBLIC_KEY) {
        switch (subclass) {
        case CKK_RSA:
...
CK_RV template_validate_attributes(struct template_list *template, CK_ULONG class,
                                   CK_ULONG subclass, CK_ULONG op_type)
{
    CK_RV rc = CKR_OK;
...
static CK_BBOOL check_rsa_privk_exportability(CK_ATTRIBUTE_TYPE type)
{
    switch (type) {
        case CKA_PRIVATE_EXPONENT:
        case CKA_PRIME_1:
...
static CK_BBOOL check_ecc_privk_exportability(CK_ATTRIBUTE_TYPE type)
{
    switch (type) {
    case CKA_VALUE:
        return FALSE;
...
static CK_BBOOL check_attr_exportability(struct template_list *tmpl_list, CK_ATTRIBUTE_TYPE type)
{
...
static CK_RV map_pkcs_to_nxp_attr(CK_ATTRIBUTE_PTR ck_attr,
                                  CK_ULONG ck_attr_count, NXP_ATTRIBUTE **nxp_attrs,
                                  uint32_t *attr_count, CK_ULONG op_type)
{
    CK_ATTRIBUTE_TYPE ck_attr_type = 0;
    CK_ATTRIBUTE_PTR ck_attrs = NULL;

    NXP_ATTRIBUTE *nxp_attr =NULL;
    NXP_OBJECT_TYPE *nxp_object_type = 0;
    NXP_KEY_TYPE *nxp_key_type = 0;

    unsigned char *nxp_label = NULL, *nxp_id = NULL;
    uint32_t *mod_bits = NULL;
    unsigned char *nxp_mod = NULL, *nxp_pub_exp = NULL;
    unsigned char *nxp_priv_exp = NULL, *nxp_prime_1 = NULL;
    unsigned char *nxp_prime_2 = NULL, *nxp_exp_1 = NULL;
    unsigned char *nxp_exp_2 = NULL, *nxp_coeff = NULL;
    unsigned char *nxp_ec_params = NULL, *nxp_ec_point = NULL;
    uint32_t *nxp_priv = NULL;
    uint32_t i = 0, attrCount = 0;
    CK_OBJECT_CLASS ck_obj_class = 0;
    CK_KEY_TYPE ck_key_type = 0;

    nxp_attr = malloc(sizeof(NXP_ATTRIBUTE) * 13);
    if (!nxp_attr)
        return CKR_HOST_MEMORY;

    memset(nxp_attr, 0, sizeof(NXP_ATTRIBUTE) * 13);
    ...
    for (i = 0; i < ck_attr_count; i++) {
        ...
        switch(ck_attr_type) {
            ...
            if (op_type == OP_CREATE) {
                ...
                switch (ck_obj_class) {
                    case CKO_PRIVATE_KEY:
                        nxp_object_type = (NXP_OBJECT_TYPE *)malloc(sizeof(NXP_OBJECT_TYPE));
                        if (!nxp_object_type)
                            return CKR_HOST_MEMORY;

                        *(nxp_object_type) = NXP_KEY_PAIR;

                        nxp_attr[attrCount].type = NXP_ATTR_OBJECT_TYPE;
                        nxp_attr[attrCount].value = nxp_object_type;
                        nxp_attr[attrCount].valueLen = sizeof(NXP_OBJECT_TYPE);
                        attrCount++;
                        break;
                    case CKO_PUBLIC_KEY:
                        nxp_object_type = (NXP_OBJECT_TYPE *)malloc(sizeof(NXP_OBJECT_TYPE));
                        ...
                        *(nxp_object_type) = NXP_PUBLIC_KEY;
            ...
            if (op_type == OP_CREATE) {
                ...
                switch (ck_key_type) {
                    case CKK_RSA:
                        nxp_key_type = (NXP_KEY_TYPE *)malloc(sizeof(NXP_KEY_TYPE));
                        if (!nxp_key_type)
                            return CKR_HOST_MEMORY;

                        *(nxp_key_type) = NXP_RSA;

                        nxp_attr[attrCount].type = NXP_ATTR_KEY_TYPE;
                        nxp_attr[attrCount].value = nxp_key_type;
                        nxp_attr[attrCount].valueLen = sizeof(NXP_KEY_TYPE);
                        attrCount++;
                        break;
            ...
            case CKA_LABEL:
                nxp_label = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_label)
                    return CKR_HOST_MEMORY;
                memset(nxp_label, 0, ck_attrs->ulValueLen);
                memcpy(nxp_label, ck_attrs->pValue, ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_OBJECT_LABEL;
                nxp_attr[attrCount].value = nxp_label;
                nxp_attr[attrCount].valueLen = ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_PRIVATE:
                nxp_priv = (uint32_t *)malloc(sizeof(uint32_t));
                if (!nxp_priv)
                    return CKR_HOST_MEMORY;
                ...
                nxp_attr[attrCount].type = NXP_ATTR_PRIVATE;
                nxp_attr[attrCount].value = nxp_priv;
                nxp_attr[attrCount].valueLen = sizeof(uint32_t);
                attrCount++;

                break;

            case CKA_ID:
                nxp_id = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_id)
                    return CKR_HOST_MEMORY;

                memset(nxp_id, 0, ck_attrs->ulValueLen);
                memcpy(nxp_id, ck_attrs->pValue, ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_OBJECT_INDEX;
                nxp_attr[attrCount].value = nxp_id;
                nxp_attr[attrCount].valueLen = ck_attrs->ulValueLen;
                attrCount++;
            ...
            case CKA_MODULUS_BITS:
                ...
                *mod_bits = *(uint32_t *)(ck_attrs->pValue);
                nxp_attr[attrCount].type = NXP_ATTR_MODULUS_BITS;
                nxp_attr[attrCount].value = mod_bits;
                nxp_attr[attrCount].valueLen = sizeof(uint32_t);
                attrCount++;

                break;
            case CKA_MODULUS:
                nxp_mod = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_mod)
                    return CKR_HOST_MEMORY;

                memset(nxp_mod, 0, ck_attrs->ulValueLen);
                memcpy(nxp_mod, ck_attrs->pValue, ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_MODULUS;
                nxp_attr[attrCount].value = nxp_mod;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_PUBLIC_EXPONENT:
                nxp_pub_exp = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_pub_exp)
                    return CKR_HOST_MEMORY;

                memset(nxp_pub_exp, 0, ck_attrs->ulValueLen);
                memcpy(nxp_pub_exp, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_PUBLIC_EXPONENT;
                nxp_attr[attrCount].value = nxp_pub_exp;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_PRIVATE_EXPONENT:
                nxp_priv_exp = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_priv_exp)
                    return CKR_HOST_MEMORY;

                memset(nxp_priv_exp, 0, ck_attrs->ulValueLen);
                memcpy(nxp_priv_exp, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_PRIVATE_EXPONENT;
                nxp_attr[attrCount].value = nxp_priv_exp;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_PRIME_1:
                nxp_prime_1 = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_prime_1)
                    return CKR_HOST_MEMORY;

                memset(nxp_prime_1, 0, ck_attrs->ulValueLen);
                memcpy(nxp_prime_1, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_PRIME_1;
                nxp_attr[attrCount].value = nxp_prime_1;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_PRIME_2:
                nxp_prime_2 = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_prime_2)
                    return CKR_HOST_MEMORY;

                memset(nxp_prime_2, 0, ck_attrs->ulValueLen);
                memcpy(nxp_prime_2, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_PRIME_2;
                nxp_attr[attrCount].value = nxp_prime_2;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_EXPONENT_1:
                nxp_exp_1 = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_exp_1)
                    return CKR_HOST_MEMORY;

                memset(nxp_exp_1, 0, ck_attrs->ulValueLen);
                memcpy(nxp_exp_1, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_EXPONENT_1;
                nxp_attr[attrCount].value = nxp_exp_1;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_EXPONENT_2:
                nxp_exp_2 = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_exp_2)
                    return CKR_HOST_MEMORY;

                memset(nxp_exp_2, 0, ck_attrs->ulValueLen);
                memcpy(nxp_exp_2, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_EXPONENT_2;
                nxp_attr[attrCount].value = nxp_exp_2;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_COEFFICIENT:
                nxp_coeff = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_coeff)
                    return CKR_HOST_MEMORY;

                memset(nxp_coeff, 0, ck_attrs->ulValueLen);
                memcpy(nxp_coeff, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_COEFFICIENT;
                nxp_attr[attrCount].value = nxp_coeff;
                nxp_attr[attrCount].valueLen = (uint32_t)ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_EC_PARAMS:
                nxp_ec_params = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_ec_params)
                    return CKR_HOST_MEMORY;

                memset(nxp_ec_params, 0, ck_attrs->ulValueLen);
                memcpy(nxp_ec_params, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_PARAMS;
                nxp_attr[attrCount].value = nxp_ec_params;
                nxp_attr[attrCount].valueLen = ck_attrs->ulValueLen;
                attrCount++;

                break;
            case CKA_EC_POINT:
                nxp_ec_point = (unsigned char *)malloc(
                    ck_attrs->ulValueLen);
                if (!nxp_ec_point)
                    return CKR_HOST_MEMORY;

                memset(nxp_ec_point, 0, ck_attrs->ulValueLen);
                memcpy(nxp_ec_point, ck_attrs->pValue,
                    ck_attrs->ulValueLen);

                nxp_attr[attrCount].type = NXP_ATTR_POINT;
                nxp_attr[attrCount].value = nxp_ec_point;
                nxp_attr[attrCount].valueLen = ck_attrs->ulValueLen;
                attrCount++;
...
static CK_RV map_nxp_to_pkcs_attr(NXP_ATTRIBUTE *nxp_attrs, CK_ATTRIBUTE_PTR *ck_attr)
{
    NXP_OBJECT_TYPE nxp_obj_type = NXP_ANY_TYPE;
    NXP_KEY_TYPE nxp_key_type;
    NXP_ATTRIBUTE_TYPE nxp_attr_type;
    ...
    nxp_attr_type = nxp_attrs[i].type;
    switch(nxp_attr_type) {
        case NXP_ATTR_OBJECT_TYPE:
            nxp_obj_type = *(NXP_OBJECT_TYPE *)nxp_attrs[i].value;
            switch (nxp_obj_type) {
                case NXP_KEY_PAIR:
                case 0x21:
                ...
                case NXP_PUBLIC_KEY:
                    ...
                    *(ck_obj_class) = CKO_PUBLIC_KEY;
                    ...
                    break;
                default:
                    print_error("Ojbect type not supported %x, %x\n" ,nxp_attr_type, nxp_obj_type);
                    return CKR_ATTRIBUTE_TYPE_INVALID;
            }
            break;
        case NXP_ATTR_KEY_TYPE:
            nxp_key_type = *(NXP_KEY_TYPE *)nxp_attrs[i].value;
        ...
        case NXP_ATTR_OBJECT_LABEL:
            ...
            memcpy(ck_label, nxp_attrs->value, nxp_attrs->valueLen);

            ck_attrs->type = CKA_LABEL;
            ck_attrs->pValue = ck_label;
            ck_attrs->ulValueLen = nxp_attrs->valueLen;
            break;
        ...
        case NXP_ATTR_OBJECT_INDEX:
            ...
            memcpy(ck_id, nxp_attrs->value, nxp_attrs->valueLen);
        ...
        case NXP_ATTR_MODULUS_BITS:
            ...
            *mod_bits = *(uint32_t *)(nxp_attrs->value);
        ...
        case NXP_ATTR_MODULUS:
            ...
            memcpy(temp, nxp_attrs->value, nxp_attrs->valueLen);
...
CK_RV find_matching_objects(CK_OBJECT_HANDLE_PTR object_handle, struct object_list *obj_list,
		                    CK_ATTRIBUTE_PTR pTemplate, CK_ULONG ulCount,
							CK_ULONG *pobjCount, CK_BBOOL private)
{
...
#define OBJ_NXP_ATTR_COUNT    2
#define RSA_PUB_NXP_ATTR_COUNT    6
#define RSA_PRIV_NXP_ATTR_COUNT    7

NXP_ATTRIBUTE_TYPE rsa_pub_attr_type[RSA_PUB_NXP_ATTR_COUNT] =
{
    NXP_ATTR_OBJECT_LABEL,
    NXP_ATTR_OBJECT_INDEX,
    NXP_ATTR_PRIVATE,
    NXP_ATTR_MODULUS,
    NXP_ATTR_PUBLIC_EXPONENT,
    NXP_ATTR_MODULUS_BITS
};

NXP_ATTRIBUTE_TYPE rsa_priv_attr_type[RSA_PRIV_NXP_ATTR_COUNT] =
{
    NXP_ATTR_OBJECT_LABEL,
    NXP_ATTR_OBJECT_INDEX,
    NXP_ATTR_PRIVATE,
...
#define ECC_PUB_NXP_ATTR_COUNT    5
#define ECC_PRIV_NXP_ATTR_COUNT    4

NXP_ATTRIBUTE_TYPE ecc_pub_attr_type[ECC_PUB_NXP_ATTR_COUNT] =
{
    NXP_ATTR_OBJECT_LABEL,
    NXP_ATTR_OBJECT_INDEX,
    NXP_ATTR_PRIVATE,
...
NXP_ATTRIBUTE_TYPE ecc_priv_attr_type[ECC_PRIV_NXP_ATTR_COUNT] =
{
    NXP_ATTR_OBJECT_LABEL,
    NXP_ATTR_OBJECT_INDEX,
    NXP_ATTR_PRIVATE,
...
static CK_RV object_add_template(OBJECT *obj, NXP_ATTRIBUTE_TYPE *nxp_attr_type, uint32_t attrCount)
{
    NXP_ATTRIBUTE temp_nxp_attr[attrCount], *nxp_attr;
    ...
    nxp_funcs = get_slot_function_list(obj->slotID);
    if (!nxp_funcs)
        return CKR_ARGUMENTS_BAD;

    memset(temp_nxp_attr, 0, sizeof(NXP_ATTRIBUTE) * attrCount);
    for (i = 0; i < attrCount; i++)
        temp_nxp_attr[i].type = nxp_attr_type[i];

    ret = nxp_funcs->NXP_GetObjectAttribute(obj->nxp_obj_handle, temp_nxp_attr, attrCount);
    if (ret != NXP_OK) {
        print_error("NXP_GetObjectAttribute failed %x\n", ret);
        return CKR_GENERAL_ERROR;
    }

    for (i = 0; i < attrCount; i++) {
        if ((int16_t)temp_nxp_attr[i].valueLen == -1)
            continue;

        nxp_attr = (NXP_ATTRIBUTE *)malloc(sizeof(NXP_ATTRIBUTE) +
            temp_nxp_attr[i].valueLen);
        if (!nxp_attr)
            return CKR_HOST_MEMORY;

        nxp_attr->type = temp_nxp_attr[i].type;
        nxp_attr->value = (uint8_t *)nxp_attr + sizeof(NXP_ATTRIBUTE);
        nxp_attr->valueLen = temp_nxp_attr[i].valueLen;

        ret = nxp_funcs->NXP_GetObjectAttribute(obj->nxp_obj_handle, nxp_attr, 1);
        if (ret != NXP_OK) {
            print_error("NXP_GetObjectAttribute failed with error code = %x\n", ret);
            free(nxp_attr);
            return CKR_GENERAL_ERROR;
        }

        rc = map_nxp_to_pkcs_attr(nxp_attr, &ck_attr);
        if (rc != CKR_OK) {
            print_error("map_nxp_to_pkcs_attr failed\n");
            free(nxp_attr);
            return CKR_GENERAL_ERROR;
        }
...
static CK_RV object_add_to_list(CK_SLOT_ID slotID, struct object_node *object)
{
...
static CK_RV create_rsa_pub_key_object(NXP_OBJECT_HANDLE hObject, struct object_node **rsa_pub_key,
		                               CK_SLOT_ID slotID)
{
    ...
    pub_key->object.nxp_obj_handle = hObject;
...
static CK_RV create_rsa_priv_key_object(NXP_OBJECT_HANDLE hObject, struct object_node **rsa_priv_key,
		                                CK_SLOT_ID slotID)
{
    ...
    priv_key->object.nxp_obj_handle = hObject;
...
static CK_RV create_ecc_pub_key_object(NXP_OBJECT_HANDLE hObject, struct object_node **ecc_pub_key,
		                               CK_SLOT_ID slotID)
{
    struct object_node *pub_key;
    CK_RV rc;
...
static CK_RV create_ecc_priv_key_object(NXP_OBJECT_HANDLE hObject, struct object_node **ecc_priv_key,
		                                CK_SLOT_ID slotID)
{
    struct object_node *priv_key;
    CK_RV rc;
...
static CK_RV create_key_object(NXP_OBJECT_HANDLE hObject, CK_ULONG class,
                               CK_ULONG subclass, struct template_list *template,
                               struct object_node **object_node, CK_SLOT_ID slotID)
{
    CK_RV rc = CKR_OK;
...
CK_RV destroy_object(CK_OBJECT_HANDLE hObject, CK_SLOT_ID slotID)
{
    ...
    if (o) {
        ...
        nxp_funcs = get_slot_function_list(slotID);
        if (!nxp_funcs)
            return CKR_ARGUMENTS_BAD;

        ret = nxp_funcs->NXP_EraseObject(obj->nxp_obj_handle);
        if (ret != NXP_OK) {
            if (ret != NXP_ERR_ITEM_NOT_FOUND) {
                print_error("NXP_EraseObject failed\n");
                return CKR_FUNCTION_FAILED;
            }
        }
...
CK_RV objects_create_object(CK_SESSION_HANDLE hSession, CK_ATTRIBUTE_PTR pTemplate,
                            CK_ULONG ulCount, CK_OBJECT_HANDLE_PTR phObject)
{
    ...
    NXP_RET_CODE nxp_ret = NXP_OK;
    NXP_ATTRIBUTE *nxp_attrs = NULL;
    NXP_OBJECT_HANDLE hObject = CK_INVALID_HANDLE;
...
#endif
    rc = map_pkcs_to_nxp_attr(pTemplate, ulCount, &nxp_attrs,
                &attrCount, op_type);
    if (rc != CKR_OK) {
        print_error("map_pkcs_to_nxp_attr failed\n");
        goto end;
    }

    nxp_ret = nxp_funcs->NXP_CreateObject(nxp_attrs,
                attrCount, &hObject);
    if (nxp_ret != NXP_OK) {
        print_error("NXP_CreateObject failed wit err code = 0x%x\n", nxp_ret);
                                ...
                                CK_OBJECT_HANDLE_PTR phPrivateKey)
{
    ...
    NXP_RET_CODE nxp_ret = NXP_OK;
    NXP_MECHANISM_INFO mechanismType = {0};
    NXP_ATTRIBUTE *nxp_attrs = NULL;
    NXP_OBJECT_HANDLE hPubObject = CK_INVALID_HANDLE, hPrivObject = CK_INVALID_HANDLE;
    uint32_t attrCount = 0;
    ...
    rc = map_pkcs_to_nxp_attr(pPublicKeyTemplate,
                ulPublicKeyAttributeCount,
                &nxp_attrs, &attrCount, op_type);
    if (rc != CKR_OK) {
        print_error("map_pkcs_to_nxp_attr failed\n");
        goto end;
    }
    ...
    switch (subclass) {
        ...
        case CKK_EC:
            mechanismType.mechanism =
                NXP_EC_PKCS_KEY_PAIR_GEN;
            break;
        default:
            print_error("Only RSA/EC Keys supported\n");
    }

    nxp_ret = nxp_funcs->NXP_GenerateKeyPair(&mechanismType, nxp_attrs,
                attrCount, &hPrivObject, &hPubObject);
    if (nxp_ret != NXP_OK) {
        print_error("NXP_GenerateKeyPair failed wit err code = 0x%x\n", nxp_ret);
        rc = CKR_GENERAL_ERROR;
        goto end;
    }
...
CK_RV get_all_token_objects(struct object_list *obj_list, CK_SLOT_ID slotID)
{
    uint32_t obj_count, j = 0;
    NXP_ATTRIBUTE temp_nxp_attr[OBJ_NXP_ATTR_COUNT];
    NXP_FUNCTION_LIST_PTR nxp_funcs = NULL;
    NXP_RET_CODE ret;
    NXP_OBJECT_HANDLE objs[MAX_FIND_LIST_OBJECTS];
    NXP_KEY_TYPE key_type = 0;
    NXP_OBJECT_TYPE obj_type = 0;
    ...
    /* For now only 50 objects are read from token and maintained
      * in PKCS library */
    ret = nxp_funcs->NXP_EnumerateObjects(NULL, 0, objs, MAX_FIND_LIST_OBJECTS, &obj_count);
    if (ret != NXP_OK) {
        print_error("NXP_EnumerateObjects failed with ret code 0x%x\n", ret);
        return CKR_GENERAL_ERROR;
    }

    for (j = 0; j < obj_count; j++) {
        memset(temp_nxp_attr, 0, sizeof(NXP_ATTRIBUTE) *
            OBJ_NXP_ATTR_COUNT);

        temp_nxp_attr[0].type = NXP_ATTR_OBJECT_TYPE;
        temp_nxp_attr[0].value = &obj_type;
        temp_nxp_attr[0].valueLen = sizeof(NXP_OBJECT_TYPE);

        temp_nxp_attr[1].type = NXP_ATTR_KEY_TYPE;
        temp_nxp_attr[1].value = &key_type;
        temp_nxp_attr[1].valueLen = sizeof(NXP_KEY_TYPE);

        ret = nxp_funcs->NXP_GetObjectAttribute(objs[j], temp_nxp_attr, 1);
        if (ret != NXP_OK) {
            print_error("NXP_GetObjectAttribute failed\n");
            return CKR_GENERAL_ERROR;
        }

        // if this is a key, get its key type
        if (obj_type == NXP_KEY_PAIR || obj_type == NXP_PUBLIC_KEY) {
            ret = nxp_funcs->NXP_GetObjectAttribute(objs[j], &temp_nxp_attr[1], 1);
            if (ret != NXP_OK) {
                print_error("NXP_GetObjectAttribute failed\n");
                return CKR_GENERAL_ERROR;
            }
...
CK_RV delete_all_token_objects(CK_SLOT_ID slotID)
{
    ...
    for (j = 0; j < obj_count; j++) {
        ret = nxp_funcs->NXP_EraseObject(objs[j]);
        if (ret != NXP_OK) {
            print_error("NXP_EraseObject failed\n");
            return CKR_GENERAL_ERROR;
        }
