...
CK_RV C_GetInfo(CK_INFO_PTR pInfo)
{
    ...
    if (cryptoki_info.manufacturerID[0] == 0) {
        ...
        strncpy((char *)cryptoki_info.manufacturerID, "NXP-Semiconductors", strlen("NXP-Semiconductors"));

        cryptoki_info.flags = 0;

        memset(cryptoki_info.libraryDescription, ' ', sizeof(cryptoki_info.libraryDescription));
        strncpy((char *)cryptoki_info.libraryDescription, "PKCS11-HSM",
            strlen("PKCS11-HSM"));

        cryptoki_info.libraryVersion.major = 0x01;
        cryptoki_info.libraryVersion.minor = 0x01;
    }
...
CK_RV C_GetFunctionList(CK_FUNCTION_LIST_PTR_PTR ppFunctionList)
{
    ...
    global_function_list.version.major          = 0x02;
