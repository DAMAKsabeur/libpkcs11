...
static void __p11_global_unlock(void *lock)
{
	if (!lock)
		return;
...
struct slot_info *get_global_slot_info(CK_SLOT_ID slotID)
{
	...
	if (slotID != NXP_S32G_HSM_SLOT_ID)
		return NULL;

	return &g_slot_info[slotID];
}

struct NXP_FUNCTION_LIST *get_slot_function_list(CK_SLOT_ID slotID)
{
	struct slot_info *s_info;

	if (slotID != NXP_S32G_HSM_SLOT_ID)
	{
		print_error("slotID %d != %d\n", (int) slotID, (int)NXP_S32G_HSM_SLOT_ID);
		return NULL;
	}
	s_info = get_global_slot_info(slotID);
	if (!s_info)
	{
		print_error("s_info is NULL \n");
		return NULL;
	}
	return g_slot_info[slotID].nxp_funcs;
}
/*
...
static int get_function_list(CK_SLOT_ID slotID, struct slot_info **ps_info)
{
	NXP_RET_CODE	rc = NXP_OK;
	struct slot_info *s_info = *ps_info;
	s_info = get_global_slot_info(slotID);
	if (!s_info) {
		print_error("get_global_slot_info failed\n");
		goto out;
	}
	rc = NXP_GetFunctionList(&s_info->nxp_funcs);
out:
	return rc;
...
CK_RV initialize_slot(CK_SLOT_ID slotID)
{
	...
	rc = get_function_list(slotID, &s_info);
	if (!rc) {
...
static uint32_t token_load_data_file(CK_SLOT_ID slotID, struct token_data *token_data)
{
	FILE *fptr = NULL;
	uint32_t rc = 0;
	char file_name[50];
	...
	sprintf(file_name, "%s%lu",  "NXP_HSM_BASED_TOKEN_", slotID);
...
static uint32_t token_save_data_file(CK_SLOT_ID slotID, struct token_data *token_data)
{
	FILE *fptr = NULL;
	uint32_t rc = 0;
	char file_name[50];
...
CK_RV token_load_data(CK_SLOT_ID slotID, struct token_data *token_data)
{
	CK_RV rc = CKR_OK;
...
CK_RV token_save_data(CK_SLOT_ID slotID, struct token_data *token_data)
{
	CK_RV rc = CKR_OK;
...
CK_RV token_get_so_pin(CK_SLOT_ID slotID, CK_UTF8CHAR_PTR pPinHash)
{
	CK_RV rc = CKR_OK;
		         ...
		         CK_ULONG ulPinLen, CK_UTF8CHAR_PTR pLabel)
{
	...
	switch (slotID) {
		case NXP_S32G_HSM_SLOT_ID:
			/* Get the Token information from NXP Token*/
			Get_NXP_TokenInfo(token_info);
			break;
		default:
			rc = CKR_SLOT_ID_INVALID;
			goto end;
